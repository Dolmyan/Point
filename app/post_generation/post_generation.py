...
async def receive_theme(message: Message, state: FSMContext):
    ...
    caption = (
        ...
        "При первой генерации я попрошу примеры твоих постов, чтобы поймать твой стиль и tone of voice.\n"
        "Нет примеров? Не страшно — можно взять чужие тексты для ориентира или выбрать наш универсальный стиль."
    )
...
async def start_content_generation(callback_query: CallbackQuery, state: FSMContext):
    # Сообщение с инструкцией для пользователя
    caption = (
        "<b>Присылай всё, чем хочешь поделиться 💭</b>\n\n"
        "☝️ Тут важно не ограничиваться сухим «коротко по теме». Я жду от тебя полный поток мыслей, которыми реально хочется поделиться с аудиторией.\n\n"
        "🚫 Плохо: «хочу рассказать про влияние алгоритмов на охваты»\n"
        "✅ Хорошо: голосом на 3–5 минут разгоняешь, как именно алгоритмы жмут охваты, что думаешь по этому поводу и какие у тебя инсайты.\n\n"
        "Можешь кидать текст или голосовое 🎤 — нейросеть всё разберёт и аккуратно упакует в тему. И не парься, если где-то собьёшься или забудешь что-то сказать — всегда можно дополнить потом.\n\n"
    )
    await bot.send_message(chat_id=callback_query.message.chat.id, text=caption, parse_mode="HTML")
    await state.set_state(Form.post_generation_input)


@router.message(Form.post_generation_input)
async def receive_post_theme(message: Message, state: FSMContext):
    post_theme = await text_or_voice(message)
    await state.update_data(post_theme=post_theme)

    # Создаём кнопки выбора платформы
    kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    # Отправляем сообщение с инструкцией и кнопками
